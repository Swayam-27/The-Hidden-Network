        ...
        <div className="about-content">
            {animate
                ? blocks.map((block, index) => {
                    ...
                    const isTypingComplete = block.typedText ? block.typedText.length === block.text.length : false;
                    const showCursor = isCurrentBlock && !isTypingComplete && block.type !== 'table';

                    if (block.type === 'h2') {
                        return <h2 key={index} className="about-content-subtitle">{block.typedText}{showCursor && <span className="typing-cursor">_</span>}</h2>;
                    }
                    if (block.type === 'p') {
                        return <p key={index} className="about-content-text">{block.typedText}{showCursor && <span className="typing-cursor">_</span>}</p>;
                    }
                    if (block.type === 'table') {
                        return (
                            <table className="ranking-table is-typing" key={index}>
                              ...
                              <tbody>
                                <tr>
                                  <td data-label="RANK" className="rank-s">S-CLASS</td>
                                  <td data-label="TITLE">GHOST PROTOCOL</td>
                                  <td data-label="REQS">Flawless precision (0 wrong attempts) & Elite speed (&lt; 1.5 min avg).</td>
                                </tr>
                                <tr>
                                  <td data-label="RANK" className="rank-a">A-CLASS</td>
                                  <td data-label="TITLE">FIELD AGENT</td>
                                  <td data-label="REQS">High precision (&lt; 50% attempts) & Fast pace (&lt; 3 min avg).</td>
                                </tr>
                                <tr>
                                  <td data-label="RANK" className="rank-b">B-CLASS</td>
                                  <td data-label="TITLE">ANALYST</td>
                                  <td data-label="REQS">Standard completion (&lt; 5 min avg).</td>
                                </tr>
                                <tr>
                                  <td data-label="RANK" className="rank-c">C-CLASS</td>
                                  <td data-label="TITLE">RECRUIT</td>
                                  <td data-label="REQS">Sloppy or slow. Performance archived.</td>
                                </tr>
                              </tbody>
                            </table>
                        );
                ...
                : missionBriefingContent.map((block, index) => {
                    if (block.type === 'h2') {
                        return <h2 key={index} className="about-content-subtitle">{block.text}</h2>;
                    }
                    if (block.type === 'p') {
                        return <p key={index} className="about-content-text">{block.text}</p>;
                    }
                    if (block.type === 'table') {
                         return (
...
const AboutPage = () => {
  ...
  const handleFinalAccessAttempt = (e) => {
    ...
    if (finalInputValue === PROFESSOR_PASSWORD) {
      ...
      setFinalInputValue('');
      setTimeout(() => { setIsFinalShaking(false); setFinalFeedback(''); }, 1000);
    }
  };
  ...
  return (
    <div className={containerClass}>
      <div>
        ...
        {step === 'success' && (
            <div id="access-terminal" className={`success-message ${step === 'revealed_briefing' ? 'fading-out' : ''}`}>
        ...
        {step === 'revealed_briefing' && (
          <div className="about-page-revealed-wrapper">
            ...
            <div className={`final-reveal-section ${isTypingFinished ? 'is-visible' : ''}`}>
              {!isFinalRevealed ? (
                <form 
                  ...
                  noValidate
                >
                    <div className="terminal-header">[ FINAL VALIDATION REQUIRED ]</div>
                    <label htmlFor="final-access-code-input">ENTER THESIS PROTOCOL:</label>
                    <p className="terminal-hint">This protocol is provided externally to the evaluator.</p>
                    <input
                      ref={finalInputRef}
                      type="password"
                    ...
                    />
                    {isMobile && (
                      <button type="submit" className="decrypt-button" style={{ marginTop: '1.5rem' }}>DECLASSIFY</button>
                    )}
                    <p id="access-feedback">{finalFeedback}</p>
                </form>
              ) : (
                <div className="about-content" style={{ marginTop: '2rem' }}>
                  <p className="about-content-text" style={{ 
                    textAlign: 'center', 
                  ...
                  <h2 className="about-content-subtitle" style={{ 
                    textAlign: 'center', 
                    fontSize: '2.5rem', 
                    marginTop: '0.5rem', 
                    marginBottom: '2rem', 
                  ...
                  <div style={{ 
                    ...
                    <p className="about-content-text"><strong>ACADEMIC CONTEXT:</strong> Government Secrecy and Intelligence Course</p>
                    <p className="about-content-text"><strong>INSTITUTION:</strong> Ahmedabad University</p>
                    <p className="about-content-text"><strong>PROJECT TYPE:</strong> Interactive Case Study</p>
                    <p className="about-content-text"><strong>BUILT WITH:</strong> React.js, GSAP, Custom CSS, LocalStorage API</p>
                    <p className="about-content-text"><strong>ACADEMIC YEAR:</strong> 2024-2025</p>
                    ...
                    <p className="about-content-text" style={{ 
                      ...
                      "The Hidden Network" is an experiential investigation into covert operations, intelligence analysis, and the signal-from-noise paradigm. The persona of "Cipher" serves as an anonymous curator throughout the archive, maintaining narrative immersion while exploring themes of secrecy, surveillance, and information warfare.
                    </p>
                  </div>
                </div>
              )}
